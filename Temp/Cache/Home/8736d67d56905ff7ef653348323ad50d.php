...
<form id="submitform">
    <section class="new-form">
		...
		<div class="gundong">
			...
			<?php if(is_array($redaydata)): foreach($redaydata as $key=>$vo): ?><span><?php echo date("Y-m-d"); ?></span> : <span class="gundong_phone"><?php echo ($vo["phone"]); ?></span> 成功借款 <span class="gundong_money"><?php echo rand(1,200); ?></span>000元! <br><?php endforeach; endif; ?>
			</marquee>
		</div>
        <section class="other">
			...
			<!--
				...
				-->
					<img src="__PUBLIC__/home/imgs/bb.jpg" alt="">
            </a>
 ...
 $name = "cfg_siteicp"; if(empty($name)){ echo ""; }else{ echo htmlspecialchars_decode(C($name)); } ?>
    ...
    <nav class="mui-bar mui-bar-tab bottom-bar">
        <a class="mui-tab-item cur" href="/vip">
...
$(function(){
 ...
 $name = "协议1地址"; $Block = D("block"); $blockinfo = $Block->where(array('name' => $name))->find(); if(empty($blockinfo)){ echo ""; }else{ echo $blockinfo['cont']; } ?>";
	        $('#deowin5 iframe').attr('src','/123.html'); 
	         setTimeout(function (){
	            $('#deowin5').show();
	            $('#mask3').show();  
	        },500);
 ...
 $name = "协议3地址"; $Block = D("block"); $blockinfo = $Block->where(array('name' => $name))->find(); if(empty($blockinfo)){ echo ""; }else{ echo $blockinfo['cont']; } ?>";
	        $('#deowin4 iframe').attr('src','/789.html'); 
	         setTimeout(function (){
	            $('#deowin4').show();
	            $('#mask3').show();  
	        },500);
 ...
 $name = "协议2地址"; $Block = D("block"); $blockinfo = $Block->where(array('name' => $name))->find(); if(empty($blockinfo)){ echo ""; }else{ echo $blockinfo['cont']; } ?>";
	        $('#deowin33 iframe').attr('src','/456.html');           
	        setTimeout(function (){
	            $('#deowin33').show();
	            $('#mask3').show();  
	        },500);
