...
<div class="mui-content padb60">
	<ul class="cf glist">
		<li>
		<!--
			<a class="cur" href="http://wpa.b.qq.com/cgi/wpa.php?ln=2&uin=<?php
 $name = "客服QQ"; $Block = D("block"); $blockinfo = $Block->where(array('name' => $name))->find(); if(empty($blockinfo)){ echo ""; }else{ echo $blockinfo['cont']; } ?>">
			-->
			<a class="cur" href="http://wpa.qq.com/msgrd?v=3&amp;uin=<?php
 $name = "客服QQ"; $Block = D("block"); $blockinfo = $Block->where(array('name' => $name))->find(); if(empty($blockinfo)){ echo ""; }else{ echo $blockinfo['cont']; } ?>&amp;site=qq&amp;menu=yes ">
